...
public class BorealisTreeFeature extends Feature<BaseTreeFeatureConfig> {

    public BorealisTreeFeature(Codec<BaseTreeFeatureConfig> codec) {
        super(codec);
    }
    ...
    private static boolean isLichen(IWorldGenerationBaseReader world, BlockPos pos) {
        return world.hasBlockState(pos, (state) -> {
            ...
            return block == BorealisBlocks.lichen_block.get();
        });
    }
    ...
    private boolean place(IWorldGenerationReader generationReader, Random rand, BlockPos positionIn, Set<BlockPos> leaves, Set<BlockPos> logs, MutableBoundingBox boundingBoxIn, BaseTreeFeatureConfig configIn) {
        ...
        if (blockpos.getY() >= 1 && blockpos.getY() + trunk + 1 <= 256) {
            if (!isLichen(generationReader, blockpos.down()) || isWater(generationReader, positionIn)) {
                return false;
            } else {
