...
public class GlacialSpireFeature extends Feature<ColumnConfig> {
    private static final ImmutableList<Block> field_236245_a_ = ImmutableList.of(Blocks.WATER, Blocks.BEDROCK, BorealisBlocks.permafrost_rubble.get(), BorealisBlocks.permafrost.get());

    public GlacialSpireFeature(Codec<ColumnConfig> codec) {
        super(codec);
    }
            ...
            if (blockpos1 != null) {
                ...
                for(BlockPos.Mutable blockpos$mutable = blockpos1.toMutable(); j >= 0; --j) {
                    if (func_236247_a_(p_236248_1_, p_236248_2_, blockpos$mutable)) {
                        this.setBlockState(p_236248_1_, blockpos$mutable, Blocks.PACKED_ICE.getDefaultState());
                        blockpos$mutable.move(Direction.UP);
                        flag = true;
                    } else {
                        if (!p_236248_1_.getBlockState(blockpos$mutable).isIn(Blocks.PACKED_ICE)) {
                            break;
                        }
    ...
    private static boolean func_236247_a_(IWorld p_236247_0_, int p_236247_1_, BlockPos p_236247_2_) {
        ...
        return blockstate.isAir() && p_236247_2_.getY() <= p_236247_1_;
    }
}
