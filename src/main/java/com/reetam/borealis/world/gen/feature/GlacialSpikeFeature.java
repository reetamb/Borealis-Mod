...
public class GlacialSpikeFeature extends Feature<NoFeatureConfig> {
    ...
    public boolean generate(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, NoFeatureConfig config) {
        BlockPos pos1 = new BlockPos(pos.getX(), reader.getHeight(Heightmap.Type.OCEAN_FLOOR_WG, pos.getX(), pos.getZ()), pos.getZ());
        pos = pos1;

        if (!reader.getBlockState(pos).isIn(Blocks.PACKED_ICE)) {
            return false;
        } else {
            ...
            for(int l1 = -k1; l1 <= k1; ++l1) {
                for(int i2 = -k1; i2 <= k1; ++i2) {
                    ...
                    while(blockpos.getY() > 50) {
                        ...
                        if (!blockstate1.isAir(reader, blockpos) && block1 != BorealisBlocks.permafrost.get() && block1 != Blocks.PACKED_ICE) {
                            break;
                        }
