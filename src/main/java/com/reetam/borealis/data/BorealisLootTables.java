    ...
    public static class Blocks extends BorealisLootTableProvider {
        ...
        protected void generate() {
            List<Block> entries = List.of(
                    ...
                    dropSelf(BorealisBlocks.BONE_DRY_WOOD),
                    dropSelf(BorealisBlocks.BONE_DRY_WOOD_BRICKS)
            );

            // catch the rest
//            Iterable<Block> knownBlocks = getKnownBlocks();
//            for (Block knownBlock : knownBlocks) {
//                BorealisMod.LOGGER.error(knownBlock.toString());
//                if (!entries.contains(knownBlock)) {
//                    dropSelf(knownBlock);
//                }
//            }
        }
